...
if not os.path.exists(undistorted_sparse_path):
    print('No sparse-txt directory for scene %s' % scene_id)
    exit()
...
if not os.path.exists(depths_path):
    print(depths_path)
    print('No depths directory for scene %s' % scene_id)
    exit()
...
if not os.path.exists(images_path):
    print('No images directory for scene %s' % scene_id)
    exit()
...
to_save = "0001"
ground_truth_matrix = np.load(f"/homes/tp4618/Documents/bitbucket/SuperGlueThesis/external/d2-net/{to_save}_overlap_matrix.npy", allow_pickle=True)
# print(loaded_overlap)
